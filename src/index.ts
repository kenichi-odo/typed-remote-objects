...
export type TROObject<ObjectName, ObjectType> = { type: ObjectName } & {
...
): Promise<TROObject<ObjectName, ObjectType>[]> {
    ...
    try {
      ;(s_object_models[object_name] as SObjectModel<ObjectType>).remote_object.retrieve(
        ...
        (error, records) => {
          ...
          resolve(
            records.map(record => {
              const result = { type: object_name } as TROObject<ObjectName, ObjectType>
    ...
    try {
      ;(s_object_models[clone_props.type] as SObjectModel<ObjectType>).remote_object.create(
        ...
        async (error, ids) => {
          ...
          resolve(_[0] as Fetch extends true ? TROObject<ObjectName, ObjectType> : void)
        },
      )
    } catch (error) {
      reject(new TROError(clone_props.type, error.message, { props, clone_props }))
